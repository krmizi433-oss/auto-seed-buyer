...
local ServerTab = Window:CreateTab("Server Changer", 4483362458)
...
local function buildServerList(statusLabel)
    serverList = {}
    isScanning = true
    local cursor = nil
    local pageCount = 0
    ...
    repeat
        ...
        for _, server in ipairs(data.data or {}) do
            ...
            if playerCount >= scSettings.minPlayers
                ...
                and (not scSettings.skipFull or playerCount < maxP)
                and (not scSettings.skipCurrent or server.id ~= currentJobId)
            then
        ...
        if statusLabel then
            statusLabel:Set("Scanning... " .. #serverList .. " servers found")
        end

        task.wait(0.3)
    until not cursor or cursor == "" or pageCount >= maxPages

    if scSettings.sortMode == "Lowest Ping" then
        table.sort(serverList, function(a, b) return a.ping < b.ping end)
    elseif scSettings.sortMode == "Most Players" then
        table.sort(serverList, function(a, b) return a.players > b.players end)
    elseif scSettings.sortMode == "Least Players" then
        table.sort(serverList, function(a, b) return a.players < b.players end)
    else
...
ServerTab:CreateSection("Status")
local scStatusLabel = ServerTab:CreateLabel("Ready — click Scan to load servers")

ServerTab:CreateButton({
    Name = "Scan Servers",
    Callback = function()
        if isScanning then
            Rayfield:Notify({ Title = "Already Scanning", Content = "Wait for the current scan to finish.", Duration = 3 })
            return
        end
        scStatusLabel:Set("Scanning...")
        task.spawn(function()
            local count = buildServerList(scStatusLabel)
            scStatusLabel:Set("Done — " .. count .. " servers found")
            Rayfield:Notify({ Title = "Scan Complete", Content = count .. " servers match your filters.", Duration = 4 })
        end)
    end,
})
...
ServerTab:CreateButton({
    ...
    Callback = function()
        if #serverList == 0 then
            Rayfield:Notify({ Title = "No Servers", Content = "Run a scan first.", Duration = 3 })
            return
        end
        local t = serverList[1]
        scStatusLabel:Set(string.format("Joining → %d/%d players | %dms ping", t.players, t.maxPlayers, t.ping))
        task.wait(0.5)
        joinServer(t.jobId)
    end,
})
...
ServerTab:CreateButton({
    ...
    Callback = function()
        ...
        local t = serverList[math.random(1, #serverList)]
        scStatusLabel:Set(string.format("Joining → %d/%d players | %dms ping", t.players, t.maxPlayers, t.ping))
        task.wait(0.5)
        joinServer(t.jobId)
    end,
...
ServerTab:CreateButton({
    ...
    Callback = function()
        scStatusLabel:Set("Rejoining...")
        task.wait(0.5)
        joinServer(currentJobId)
    end,
})
...
ServerTab:CreateSlider({
    Name = "Min Players",
    Range = {1, Players.MaxPlayers},
    Increment = 1,
    Suffix = "players",
    CurrentValue = scSettings.minPlayers,
    Flag = "SC_MinPlayers",
    Callback = function(val) scSettings.minPlayers = val end,
})
...
ServerTab:CreateSlider({
    ...
    Increment = 1,
    Suffix = "players",
    CurrentValue = scSettings.maxPlayers,
    Flag = "SC_MaxPlayers",
    Callback = function(val) scSettings.maxPlayers = val end,
})

ServerTab:CreateToggle({
    Name = "Skip Full Servers",
    CurrentValue = true,
    Flag = "SC_SkipFull",
    Callback = function(val) scSettings.skipFull = val end,
})

ServerTab:CreateToggle({
    Name = "Skip Current Server",
    CurrentValue = true,
    Flag = "SC_SkipCurrent",
    Callback = function(val) scSettings.skipCurrent = val end,
})

ServerTab:CreateDropdown({
    Name = "Sort Servers By",
    Options = {"Random", "Lowest Ping", "Most Players", "Least Players"},
    CurrentOption = {"Random"},
    Flag = "SC_SortMode",
    Callback = function(opts) scSettings.sortMode = opts[1] end,
})
